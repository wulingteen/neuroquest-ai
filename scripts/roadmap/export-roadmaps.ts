 ...
 *
 * Usage:
 *   npm run tree -- <slug>                  # ASCII tree (single roadmap)
 *   npm run tree -- <slug> --json           # JSON output
 *   npm run tree -- <slug> --json --pretty  # pretty JSON
 *   npm run tree -- <slug> -o tree.json     # write to file
 *
 * Topic content lookup:
 *   npm run tree -- <slug> <label>          # print content for a label
 *   npm run tree -- react "Vite"            # exact or substring match
 *   npm run tree -- react vite              # case-insensitive
 *
 * Cross-roadmap relationship graph:
 *   npm run graph -- <slug> [slug2 ...]     # ASCII graph of related roadmaps
 *   npm run graph -- --all                  # graph of ALL roadmaps
 *   npm run graph -- <slug> --depth 2       # limit traversal depth
 *   npm run graph -- <slug> --json          # JSON output
 */
...
function main() {
    ...
    if (graphMode) {
        ...
        if (targetSlugs.length === 0) {
            console.error(
                'Usage: npm run graph -- <slug> [slug2 ...] [--all] [--depth N] [--json] [--pretty] [-o <file>]\n',
            );
    ...
    if (slugs.length === 0) {
        console.error(
            'Usage: npm run tree -- <slug> [<label>] [--json] [--pretty] [-o <file>]\n',
        );
        console.error('  Topic content lookup:');
        console.error(
            '    npm run tree -- <slug> "<label>"\n',
        );
        console.error('  Cross-roadmap graph:');
        console.error(
            '    npm run graph -- <slug> [slug2 ...] [--all] [--depth N]\n',
        );
        console.error('Available roadmaps:\n');
        for (const s of allSlugs) console.error(`   ${s}`);
        process.exit(1);
